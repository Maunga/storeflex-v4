...
import { PageProps, SearchResultItem } from '@/types';
                        ...
                        <div className="max-w-6xl mx-auto animate-page-enter">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                {results.map((item, index) => (
                                    ...
                                    >
                                        {/* Image */}
                                        <div className="relative aspect-square bg-neutral-50 dark:bg-neutral-800 flex items-center justify-center p-4 overflow-hidden">
                                            {item.url_image ? (
                                                <img
                                                    src={item.url_image}
                                                    alt={item.title}
                                                ...
                                                <svg className="w-16 h-16 text-neutral-300 dark:text-neutral-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1">
                                            ...
                                            {item.is_prime && (
                                                <span className="absolute top-2 left-2 px-1.5 py-0.5 text-[10px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/40 dark:text-blue-400 rounded">
                                                    Prime
                                                </span>
                                            )}
                                            {item.best_seller && (
                                                <span className="absolute top-2 right-2 px-1.5 py-0.5 text-[10px] font-bold text-orange-600 bg-orange-50 dark:bg-orange-900/40 dark:text-orange-400 rounded">
                                                    Best Seller
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        <div className="p-4 space-y-2">
                                            ...
                                            {item.rating != null && (
                                                <div className="flex items-center gap-1.5">
                                                    <div className="flex">
                                                        {[1, 2, 3, 4, 5].map((s) => (
                                                            <svg key={s} className={`w-3.5 h-3.5 ${s <= Math.round(item.rating!) ? 'text-yellow-400' : 'text-neutral-300 dark:text-neutral-600'}`} viewBox="0 0 20 20" fill="currentColor">
                                            ...
                                            <div className="flex items-baseline gap-2">
                                                ...
                                                {item.price_strikethrough != null && item.price_strikethrough > 0 && (
                                                    <span className="text-xs text-neutral-400 line-through">
                                                        {item.currency ?? 'AED'} {item.price_strikethrough}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            {item.shipping_information && (
                                                <p className="text-[11px] text-neutral-400 dark:text-neutral-500 leading-snug line-clamp-1">
                                                    {item.shipping_information}
                                                </p>
                                            )}
                                        </div>
